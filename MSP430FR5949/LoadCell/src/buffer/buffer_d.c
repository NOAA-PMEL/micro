...
{
	uint8_t NextWrite = (buf->write + 1) % (ACTUAL_BUFFER_D_SIZE);
...
int8_t BufferD_Get(CircularBufferD_s *buf, double *value)
{
	...
	*value = buf->buffer[buf->read];
	buf->read = (buf->read + 1) % ACTUAL_BUFFER_D_SIZE;
...
int8_t BufferD_Clear(CircularBufferD_s *buf)
{
	...
	for(i=0;i<ACTUAL_BUFFER_D_SIZE;i++)
	{
		buf->buffer[i] = 0;
	}
...
{
	uint16_t NextWrite = (buf->write + 1) %(ACTUAL_BUFFER_D_SIZE);
