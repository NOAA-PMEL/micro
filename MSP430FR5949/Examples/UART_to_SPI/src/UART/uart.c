...
uint8_t UART_Read(char *value, uint8_t Port)
{
    /* Check for valid Port # */
	if(Port > 1)
	{
...
uint8_t UART_WriteChar(char value, uint8_t Port)
{
    /* Check for valid Port # */
	if(Port > 1)
	{
...
uint8_t UART_Write(char *value, uint8_t length, uint8_t Port)
{
	uint8_t i=0;
...
__interrupt void USCI_A1_ISR(void)
{
	switch(__even_in_range(UCA1IV, USCI_UART_UCTXCPTIFG))
	{
		case USCI_NONE:
			break;
		case USCI_UART_UCRXIFG:     
            BufferC_Put(&RxBuf,(char) UCA1RXBUF);
            break;
		case USCI_UART_UCTXIFG:
            DMA_TriggerBlock();
